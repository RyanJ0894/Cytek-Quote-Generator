...
const DEFAULT_KEY = "default_data_source_id";
...
export class PgDataSourceStore implements DataSourceStore {
  ...
  async delete(id: string): Promise<boolean> {
    ...
    if (rows.length && (await this.getDefaultId()) === id) await this.setDefaultId(null);
    return rows.length > 0;
  }

  async getDefaultId(): Promise<string | null> {
    await this.ensureSchema();
    const { rows } = await this.client.query(`SELECT value FROM app_settings WHERE key = $1`, [DEFAULT_KEY]);
    return (rows[0] as { value: string } | undefined)?.value ?? null;
  }

  async setDefaultId(id: string | null): Promise<void> {
    await this.ensureSchema();
    if (id === null) {
      await this.client.query(`DELETE FROM app_settings WHERE key = $1`, [DEFAULT_KEY]);
      return;
    }
    await this.client.query(

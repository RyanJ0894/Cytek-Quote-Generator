...
## Deploying `api-server` to Vercel — Don't Trust the "Express" Zero-Config Preset

Vercel's "Express" Application Preset does more than run `pnpm run build`. *After* that build succeeds (you'll see `dist/index.cjs` get produced correctly), Vercel separately walks the **TypeScript source** itself to auto-construct a serverless function, using its own forked `ts-node`-style compiler (`@vercel/node`'s `typescript.ts`, which compiles per-file via the TS language service, printing `Using TypeScript X.X.X (local user-provided)` before it runs). That compiler force-overrides `moduleResolution` to `NodeNext`, regardless of what `tsconfig.json` specifies.

This monorepo's `lib/*` packages resolve via `"exports": { ".": "./src/index.ts" }` — pointing straight at TypeScript source, which is exactly the pattern `moduleResolution: "bundler"` (used everywhere in this repo) is designed to support, but which `NodeNext` resolution does not handle the same way (real Node.js can't execute raw `.ts`, and `NodeNext` mode mimics real Node resolution more strictly). The result: Vercel's forced-`NodeNext` compile of our source can't correctly resolve `@workspace/api-zod`/`@workspace/config`, and fails with an opaque `Error: <file>: Emit skipped` — reported against whatever file it happened to be processing (consistently `src/routes/health.ts`, since it's the first module imported by `routes/index.ts`). This has nothing to do with the actual code in that file, or even actual type errors in the project — it's Vercel's own auto-detection compiling our source under incompatible settings.

**The fix (already in place): don't let Vercel compile our TypeScript at all.** `artifacts/api-server/src/vercel.ts` is a tiny wrapper that re-exports the Express `app` (from `app.ts`, no `.listen()` call). `build.ts` bundles it via esbuild into `dist/vercel.cjs` — already-compiled plain JavaScript, alongside the normal `dist/index.cjs` used by Replit/persistent-process hosts. `artifacts/api-server/vercel.json` points Vercel directly at that pre-built file (`functions: { "dist/vercel.cjs": ... }` + a catch-all `rewrite`), which sidesteps Vercel's automatic Express/TypeScript detection entirely — by the time Vercel touches this function, it's plain JS, so its TypeScript compiler never runs. `buildCommand` in `vercel.json` still runs our normal `pnpm run build` first, so this bundle is always freshly built from source, not committed to the repo.

If Vercel deployment breaks again with an `Emit skipped` or similar opaque TypeScript error, check `vercel.json` is still present and still pointing at a pre-built `.cjs`/`.js` file — not TypeScript source — before assuming it's an actual bug in the application code.

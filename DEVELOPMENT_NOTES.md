...
- **Switching.** `<html class="dark">` selects the dark tokens. The inline script in `index.html` applies it before first paint from `localStorage["eqg-theme"]`, else `prefers-color-scheme`, so returning users see no flash. `src/hooks/use-theme.ts` (`useTheme`, `useThemeSync`) owns the runtime: the toggle saves the choice; while nothing is saved the app follows OS changes live; other tabs stay in step via the `storage` event. `ThemeToggle` in `AppHeader` is the only control.

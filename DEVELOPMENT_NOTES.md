...
**Terms & Conditions** are part of the profile (`termsAndConditions`: title, subtitle, intro, `sections[{heading, body}]`, up to 50 sections, bodies as multi-paragraph text where a blank line separates paragraphs and single line breaks are kept). `termsConfigured(profile)` is the single rule for "Configured": an intro or at least one provision with text; a title alone is not terms. `summarizeQuoteProfile` exposes `hasTerms` and `termsSectionCount` for the badges. The Cytek contractual language (14 provisions, "GENERAL TERMS AND CONDITIONS OF SALE (TIME AND MATERIALS)") came from the pre-existing `lib/config/src/companies/cytek.ts` and is seeded unchanged into the Cytek — Current profile (`cytek/profile.ts`). Quote-specific text stays in the request's `notes` (Additional Notes); a future per-quote terms override would be a `termsAndConditions` field on `QuoteRequest` that `routes/quotes.ts` uses in place of the profile's, nothing else needs to change.
